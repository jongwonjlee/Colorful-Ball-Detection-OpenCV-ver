...
int main()
{
    ...
    while (cap.isOpened()){
        ...
        */
        ...
        cout << "detected red balls: " << contours_r.size() << endl;
        cout << "detected blue balls: " << contours_b.size() << endl;
        ...
        for( size_t i = 0; i< contours_r.size(); i++ ){
            if (radius_r[i] > iMin_tracking_ball_size){ //if radius_r[i] is larger than iMin_tracking_ball_size
                ...
                drawContours(result, contours_r_poly, static_cast<int>(i), color, 3, 8, vector<Vec4i>(), 0, Point());

                //calculate balls' 3d position and print it on the window
                vector<float> ball_position_r; //declare float vector named ball_position_r
                ball_position_r = pixel2point(center_r[i], radius_r[i]);
                //put position info to ball_position_r processed by function; pixel2point
        ...
        for( size_t i = 0; i< contours_b.size(); i++ ){ //run the loop while size_t type i from 0 to size of contours_b-1 size by increasing i 1
            if(radius_b[i] > iMin_tracking_ball_size){ //if radius_b[i] satisfies the condition that its size is larger than iMin_tracking_ball_size (in pixel size)
                ...
                drawContours(result, contours_b_poly, static_cast<int>(i), color, 3, 8, vector<Vec4i>(), 0, Point());

                //draw
                vector<float> ball_position_b; //declare float vector named ball_position_b
                ball_position_b = pixel2point(center_b[i], radius_b[i]);
                //put position info to ball_position_b processed by function; pixel2point
